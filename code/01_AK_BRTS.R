...
# RELATIONSHIPS IN ALASKA FROM 1950-2009. THE SCRIPT USES THE gbm, raster, and      
...
save_dir <- paste(wdir,"AK_FINAL_RESULTS/",sep="")

# CREATE DIRECTORY, IF IT DOES NOT ALREADY EXIST, TO STORE RESULTS
direxist <- dir.exists(paste(wdir,save_dir,sep = ""))
if (direxist == FALSE){
  dir.create(save_dir,
             recursive = TRUE)
}
